...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    const result = await quotationApplicationService.confirm(quotationId, actor);
    ...
    const status = err?.message?.includes("Access denied")
      ...
      ? 404
      : err?.message?.includes("Cannot confirm")
      ? 400
      : 500;
    return apiError("QUOTATION_CONFIRM_ERROR", err?.message || "Failed to confirm quotation", status);
  }
}
